...
final dio = Dio(
    BaseOptions(
      /// Since this project use multiple base url (one for weather and one for
      /// local city list, baseUrl parameter will not specified
      //   baseUrl: apiBaseUrl,
        headers: {
...
class DioService {
  ...
  Future<ResponseModel<R>> post<R>({
    ...
    //     .catchError((er){
    //   return Future<Response<Map<String, String>>>.error(er);
    // });
    return ResponseModel<R>.fromJson(response.data!);
  }
